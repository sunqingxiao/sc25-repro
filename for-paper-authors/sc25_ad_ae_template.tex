...
%\artsampl{
\textcolor{red}{Taking  Ascend NPU cluster as an example, the expected reproduction time of artifact $A_1$ is about 2030 minutes. This includes 10 minutes for $C_1$, 20 minutes for $C_2$, and 2000 minutes for $C_3$.}
...
\textcolor{red}{We conduct experiments in the software environment configured with vLLM v0.18.0 and xLLM v0.8.0. We use the vllm-ascend porting for the Ascend cluster. }
...
\textcolor{red}{For ease of installation, we provide an \texttt{install.sh} script to download the required container images and model weights. It downloads four images for vLLM and xLLM on NPU and GPU, respectively, as well as the Qwen model weights in three sizes.}
...
There are three tasks included in the Artifact Execution: conducting experiments to evaluation the collaborative performance of attention computation and beam search ($T_1$), \textcolor{red}{conducting experiments to evaluate the efficiency of scheduling optimization ($T_2$)}, conducting experiments to evaluate the end-to-end performance under strict P99 latency contraints ($T_3$). Each task involves actual program execution and result analysis. The execution scripts automatically call the plotting code in the /plot directory and saves the corresponding figures in the current directory.

\textcolor{red}{In Task $T_1$, users should create the xLLM-NPU container and run \texttt{fig16/run\_fig16\_pipeline.sh} to reproduce the corresponding results.}

\textcolor{red}{In Task $T_2$, users should create the xLLM-NPU container and run \texttt{fig15/run\_fig15\_pipeline.sh} to reproduce the corresponding results.}

\textcolor{red}{In Task $T_3$ on NPU, users should create the vLLM-Ascend and xLLM-NPU containers and run \path{fig13/run_fig13_pipeline.sh} to reproduce the corresponding results.}

\textcolor{red}{In Task $T_3$ on GPU, users should create the vLLM-GPU and xLLM-GPU containers and run \path{fig18/run_fig18_pipeline.sh} to reproduce the corresponding results.}
